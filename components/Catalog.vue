<template>
  <div :class="$style.catalog">
    <div :class="$style.sort">
      <span :class="$style.sort__label">Сортировать по:</span>
      <span
        :class="$style.sort__result"
        @click="showFilter ? (showFilter = false) : (showFilter = true)"
      >
        {{ filterLabel }}
        <span :class="$style.sort__control">
          <svg
            v-if="!showFilter"
            width="10"
            height="7"
            viewBox="0 0 5 3"
            fill="none"
            xmlns="http://www.w3.org/2000/svg"
          >
            <path d="M5 0H0L2.5 2.5L5 0Z" fill="#59606D" />
          </svg>
          <svg
            v-else
            width="10"
            height="7"
            viewBox="0 0 5 3"
            fill="none"
            xmlns="http://www.w3.org/2000/svg"
          >
            <path
              transform="rotate(-180 2.5,1.2499999999999998)"
              d="M5 0H0L2.5 2.5L5 0Z"
              fill="#59606D"
            />
          </svg>
        </span>
      </span>
      <div :class="$style.select" v-if="showFilter">
        <div :class="$style.select__option" @click="sortByPrice()">По цене</div>
        <div :class="$style.select__option" @click="sortByRating()">
          По популярности
        </div>
      </div>
    </div>
    <transition>
      <div :class="$style.catalog__content">
        <div
          :class="[$style.catalog__card, $style.card]"
          v-for="card in this.products"
          :key="card.id"
        >
          <div :class="$style.card__rating">
            <svg
              width="20"
              height="20"
              viewBox="0 0 20 20"
              fill="none"
              xmlns="http://www.w3.org/2000/svg"
            >
              <rect width="20" height="20" fill="white" />
              <path
                fill-rule="evenodd"
                clip-rule="evenodd"
                d="M10 3.125C10.2379 3.125 10.4552 3.26005 10.5605 3.47336L12.3464 7.09154L16.3404 7.67532C16.5758 7.70972 16.7712 7.87473 16.8445 8.10098C16.9179 8.32723 16.8565 8.57552 16.6861 8.74147L13.7966 11.5559L14.4785 15.5318C14.5187 15.7663 14.4223 16.0033 14.2299 16.1432C14.0374 16.283 13.7822 16.3014 13.5716 16.1907L10 14.3124L6.42843 16.1907C6.21785 16.3014 5.96267 16.283 5.77018 16.1432C5.5777 16.0033 5.48129 15.7663 5.52151 15.5318L6.20344 11.5559L3.31394 8.74147C3.14355 8.57552 3.08213 8.32723 3.15549 8.10098C3.22885 7.87473 3.42428 7.70972 3.65963 7.67532L7.6536 7.09154L9.43958 3.47336C9.54487 3.26005 9.76213 3.125 10 3.125ZM10 5.16203L8.62921 7.93914C8.53825 8.12342 8.36251 8.25121 8.15916 8.28093L5.09278 8.72913L7.3111 10.8898C7.45852 11.0334 7.52581 11.2403 7.49102 11.4432L6.96763 14.4948L9.70911 13.0531C9.89122 12.9573 10.1088 12.9573 10.2909 13.0531L13.0324 14.4948L12.509 11.4432C12.4742 11.2403 12.5415 11.0334 12.6889 10.8898L14.9073 8.72913L11.8409 8.28093C11.6375 8.25121 11.4618 8.12342 11.3708 7.93914L10 5.16203Z"
                fill="#F2C94C"
              />
              <path
                v-if="card.rating > 0"
                d="M6.5 11L4 8H10H16L13.5 11V14.5L13 15.5L10 14L9 14.5L6.5 15.5V14.5V11Z"
                fill="#F2C94C"
              />
            </svg>
            <span :class="$style.card__rating__text">{{ card.rating }}</span>
          </div>
          <div
            :class="
              checkProductsInCart(card) || checkProductsIdInCart(card)
                ? [$style.card__btn, $style.card__btn__active]
                : $style.card__btn
            "
            @click="addToCart(card)"
          ></div>
          <div :class="$style.card__img__wrapper">
            <img
              :class="$style.card__img"
              :src="'https://frontend-test.idaproject.com' + card.photo"
              @error="imgError()"
              :alt="card.name"
            />
          </div>
          <h3 :class="$style.card__title">{{ card.name.toLowerCase() }}</h3>
          <span :class="$style.card__price">{{ card.price }} ₽</span>
        </div>
      </div>
    </transition>
  </div>
</template>

<script>
export default {
  props: ["category"],
  data() {
    return {
      showFilter: false,
      filterLabel: "цене",
      productsInCartId: [],
    };
  },
  computed: {
    products() {
      if (this.category) {
        return this.$store.getters["products/products"].filter(
          (c) => c.category + "" == this.category
        );
      } else return this.$store.getters["products/products"];
    },
    productsInCart() {
      return this.$store.getters["products/productsInCart"];
    },
  },
  methods: {
    async addToCart(card) {
      if (this.productsInCart.find((f) => f.id === card.id) === undefined) {
        await this.$store.dispatch("products/addToCart", card);
        event.target.classList.toggle("trigger");
      } else {
        await this.$store.dispatch("products/deleteFromCart", card);
        event.target.classList.toggle("cancel");
        this.productsInCartId = this.productsInCartId.filter(
          (c) => c != card.id
        );
      }
    },
    async sortByRating() {
      await this.$store.dispatch("products/sortByRating");
      this.filterLabel = "популярности";
      this.showFilter = false;
    },
    async sortByPrice() {
      await this.$store.dispatch("products/sortByPrice");
      this.filterLabel = "цене";
      this.showFilter = false;
    },
    imgError() {
      event.target.src = require("@/assets/img/default.jpg");
    },
    checkProductsInCart(card) {
      if (this.productsInCart.includes(card)) {
        return true;
      } else return false;
    },
    checkProductsIdInCart(card) {
      if (this.productsInCartId.includes(card.id)) {
        return true;
      } else return false;
    },
  },
  mounted() {
    if (this.$store.getters["products/products"].length === 0) {
      this.$store.dispatch("products/fetchProducts");
    }
    if (this.productsInCart.length) {
      this.productsInCart.forEach((c) => this.productsInCartId.push(c.id));
    }
  },
};
</script>

<style lang="scss" module>
@import "./assets/scss/catalog.scss";
</style>

<style lang="scss" scoped>
@import "./assets/scss/animations.scss";
</style>
